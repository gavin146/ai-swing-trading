...
export function buildMorningEmailAlert(args: {
  ...
  const rows = top
    .map(
      (item) => `
        <tr>
          <td style="padding:12px;border-bottom:1px solid #e6ece8;font-weight:700;">${item.symbol}</td>
  ...
  const textPicks = top
    .map(
      (item, index) =>
        `${index + 1}. ${item.symbol}: score ${item.score}, confidence ${item.confidence}, risk ${item.risk_score}, entry $${item.entry_low}-${item.entry_high}, target $${item.target_price}, stop $${item.stop_loss}`,
    )
    .join("\n");
  ...
  return {
    ...
    html: `
      <div style="font-family:Inter,Arial,sans-serif;color:#17211d;line-height:1.5;max-width:760px;margin:0 auto;">
        <h1 style="font-size:28px;margin:0 0 8px;">TradePilot AI morning brief</h1>
        <p style="margin:0 0 20px;color:#52615b;">Good morning ${args.customerName}. Market regime: <strong>${args.marketRegime}</strong>.</p>
